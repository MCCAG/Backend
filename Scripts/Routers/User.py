...
async def share(image_id: ImageId, background: Background) -> dict:
    ...
    分享一个头像。
    """
    if head_image := await cache_collection.find_one({'_id': image_id}):
        data = {
            'background': background, 'likes': 0, 'users': '',
            '_id': image_id, 'time': head_image['time'],  'image': head_image['image']
        }
        await image_collection.insert_one(data)
        return {'success': True}
...
async def like(image_id: ImageId, user_id: UserId) -> dict:
    ...
    点赞一个头像。
    """
    if head_image := await image_collection.find_one({'_id': image_id}, {'likes': 1, 'users': 1}):
        ...
        await image_collection.update_one({'_id': image_id}, {'$set': {'likes': new_likes, 'users': new_users}})
        return {'success': True}
